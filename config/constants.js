export const BASE_DEV_URL = process.env.NEXT_PUBLIC_URL
import message from '../config/message.json'
import userType from '../helpers/userType'

export const menus = ({ trans, role }) => {
  const { isAdmin, isSchool, isParent, isStudent, isEmployee, isrelative } =
    userType(role)

  const schoolMenu = [
    {
      name: trans?.navbar?.dashboard || 'Dashboard',
      url: '/dashboard',
      icon: '/icons/dashboard',
      ext: '.svg',
    },

    {
      name: trans?.navbar?.classroom || 'Classrooms',
      url: '/dashboard/classes',
      icon: '/icons/classes',
      ext: '.svg',
    },

    {
      name: trans?.navbar?.students || 'Students',
      url: '/dashboard/students',
      icon: '/icons/students',
      ext: '.svg',
    },

    {
      name: trans?.navbar?.fees || 'Fees',
      url: '/dashboard/fees',
      icon: '/icons/fees',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.employees || 'Employees',
      url: '/dashboard/employees',
      icon: '/icons/employee',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.positions || 'Positions',
      url: '/dashboard/positions',
      icon: '/icons/position',
      ext: '.svg',
    },

    {
      name: trans?.navbar?.a || 'Deductibles',
      url: '/dashboard/deductible-types',
      icon: '/icons/enumarations',
      ext: '.svg',
    },

    {
      name: trans?.navbar?.installments || 'installments',
      url: '/dashboard/school-installment',
      icon: '/icons/installment',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.payments || 'Payments history',
      url: '/dashboard/payments-history',
      icon: '/icons/payment',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.messages || 'Messages',
      url: '/dashboard/messages',
      icon: '/icons/message',
      ext: '.svg',
    },
  ]

  const adminMenu = [
    {
      name: trans?.navbar?.dashboard || 'Dashboard',
      url: '/dashboard',
      icon: '/icons/dashboard',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.schools || 'Schools',
      url: '/dashboard/schools',
      icon: '/icons/schools',
      ext: '.svg',
    },

    {
      name: trans?.navbar?.academic_years || 'Academic years',
      url: '/dashboard/academic-years',
      icon: '/icons/academic',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.payments || 'Payments history',
      url: '/dashboard/payments-history',
      icon: '/icons/payment',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.messages || 'Messages',
      url: '/dashboard/messages',
      icon: '/icons/message',
      ext: '.svg',
    },
  ]

  const parentMenu = [
    {
      name: trans?.navbar?.children || 'Children',
      url: '/dashboard',
      icon: '/icons/children',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.paynow || 'Pay now',
      url: '/dashboard/parent-pay-now',
      icon: '/icons/payment',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.payments || 'Payments history',
      url: '/dashboard/parent-pay-history',
      icon: '/icons/payment',
      ext: '.svg',
    },

    {
      name: trans?.navbar?.installments || 'installments',
      url: '/dashboard/installments',
      icon: '/icons/installment',
      ext: '.svg',
    },
  ]

  const relativeMenu = [
    {
      name: trans?.navbar?.children || 'Children',
      url: '/dashboard',
      icon: '/icons/children',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.paynow || 'Pay now',
      url: '/dashboard/parent-pay-now',
      icon: '/icons/payment',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.payments || 'Payments history',
      url: '/dashboard/parent-pay-history',
      icon: '/icons/payment',
      ext: '.svg',
    },

    {
      name: trans?.navbar?.installments || 'installments',
      url: '/dashboard/installments',
      icon: '/icons/installment',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.students || 'Students',
      url: '/dashboard/students',
      icon: '/icons/students',
      ext: '.svg',
    },
  ]

  const studentMenu = [
    {
      name: trans?.navbar?.my_profile || 'My profile',
      url: '/dashboard',
      icon: '/icons/student-profile',
      ext: '.svg',
    },
    ,
    {
      name: trans?.navbar?.paynow || 'Pay now',
      url: '/dashboard/pay-now',
      icon: '/icons/payment',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.payments || 'Payments history',
      url: '/dashboard/payments',
      icon: '/icons/payment',
      ext: '.svg',
    },

    {
      name: trans?.navbar?.installments || 'installments',
      url: '/dashboard/installments',
      icon: '/icons/installment',
      ext: '.svg',
    },
  ]

  const employeeMenu = [
    {
      name: trans?.navbar?.my_profile || 'My profile',
      url: '/dashboard',
      icon: '/icons/student-profile',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.enumarations || 'Enumarations',
      url: '/dashboard/enumarations',
      icon: '/icons/enumarations',
      ext: '.svg',
    },
    {
      name: trans?.navbar?.deductibles || 'Deductibles',
      url: '/dashboard/deductibles',
      icon: '/icons/deductible',
      ext: '.svg',
    },
  ]

  return isAdmin
    ? adminMenu
    : isParent
    ? parentMenu
    : isrelative
    ? relativeMenu
    : isStudent
    ? studentMenu
    : isEmployee
    ? employeeMenu
    : schoolMenu
}

export const login_options = {
  parent: 'PARENT',
  school: 'SCHOOL',
  admin: 'ADMIN',
  student: 'STUDENT',
  employee: 'EMPLOYEE',
}

export const _ns_token_ = '_nest_school_logged_in_'
export const _pagination_number_ = 10
export const _selected_lang_ = '_selected_lang_'

export const termOptions = [
  { key: 1, value: 'TERM1', label: 'Term I' },
  { key: 2, value: 'TERM2', label: 'Term II' },
  { key: 3, value: 'TERM3', label: 'Term III' },
]

export const available_langs = message.languages

export const countries = [
  { index: 'Rwanda', value: 'Rwanda', label: 'Rwanda' },
  { index: 'Kenya', value: 'Kenya', label: 'Kenya' },
  { index: 'Burundi', value: 'Burundi', label: 'Burundi' },
  { index: 'Tanzania', value: 'Tanzania', label: 'Tanzania' },
]

export const GLOBAL_IMAGE_FALLBACK = `data:image/png;base64,`
