...
const studentsEndpoints = baseAPI.injectEndpoints({
	endpoints: (builder) => ({
		...
		paymentHistory:builder.query({
			...
			query: ({ page, size, id,academicYearId, academicTerm,from,to }) => ({
				url: `students/${id}/payments?page=${page || ""}&limit=${size || ""}&academicYearId=${academicYearId}&academicTerm=${academicTerm}&from=${from}&to=${to}`,
				method: "GET",
			}),
		})
